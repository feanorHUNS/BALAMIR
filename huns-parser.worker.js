...
// ── Web Worker girisi ──────────────────────────────────────────────────────
// Ayristirma ana is parcaciginda DEGIL burada calisir: 200 KB'lik bir dosyayi
// cozmek sekmeyi dondurmasin, bozuk bir girdi sonsuz donguye girse bile
// arayuz yanit vermeye devam etsin.
//
// Buradaki sonuc SADECE ONIZLEME icindir. Sunucu dosyayi sifirdan yeniden
// ayristirir ve yalnizca kendi sonucunu kaydeder.
self.onmessage = function (e) {
  try {
    const r = parseConfigFile(e.data);
    self.postMessage({ ok: true, result: {
      owner: r.owner,
      slots: r.slots.map(s => ({
        slot: s.slot, ok: s.ok, issues: s.issues, bytes: s.bytes,
        summary: s.match ? {
          serial:  s.match.serial,
          mapName: s.match.mapName,
          mapSys:  s.match.mapSys,
          startMs: s.match.startMs,
          durMs:   s.match.durMs,
          players: s.match.players.map(p => ({ name: p.name, cls: p.cls, team: p.team })),
          damage:  s.match.damage.length,
          heal:    s.match.heal.length,
          deaths:  s.match.deaths.length,
          points:  s.match.points.length,
          positions: s.match.positions.length,
          distances: s.match.distances.length,
        } : null,
      })),
    }});
  } catch (err) {
    self.postMessage({ ok: false, code: err.code || 'E-FORMAT',
